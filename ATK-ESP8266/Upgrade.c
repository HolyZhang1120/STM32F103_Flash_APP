...
//	u8 *size_t;
	...
	while(send_server(crc32_send((u8 *)buffer_send,3)))
		return NULL;
	if(USART3_RX_BUF1[0]==0xAA)
	{
		if(USART3_RX_BUF1[1]==0xAB)
		{
			if(crc32_rev((u8 *)USART3_RX_BUF1, 14))
			{
...
//	}
	if(USART3_RX_BUF1[0]==0xAA)
	{
		if(USART3_RX_BUF1[1]==0xAC)
		{
			if(crc32_rev((u8 *)USART3_RX_BUF1, size_num))
			{
